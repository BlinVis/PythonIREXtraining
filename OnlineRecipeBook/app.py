...
from lesson23.connect import response, dev_data
...
def create_category(category_name):
    responses=requests.post(f"{Api_base_url}/categories",json={'name':category_name})
    return responses.json
def update_category(category_id,category_name):
    response=requests.put(f"{Api_base_url}/categories/{category_id}",json={"name":category_name})
    return response.json()
...
def update_recipe(recipe_id,recipe_name,description,ingredients,instructions,cuisine,difficulty,category_id):
    response = requests.post(f"{Api_base_url}/recipes/{recipe_id}", json={
...
if selected_menu=='dashboard':
    ...
    if category_list:
        ...
        recipe_difficulty=st.selectbox("Diffivulty",["easy","medium","hard"],key="new_recipe_difficulty")
        category_list=get_categories()
        if category_list:
        ...
        if st.button("Add recipe",key="add_recipe_button"):
            if (all([recipe_name,recipe_ingredients,recipe_cuisine,recipe_instructions,recipe_difficulty,selected_category_id is not None])):
                create_recipes(recipe_name,recipe_ingredients,recipe_cuisine,recipe_instructions,recipe_difficulty,selected_category_id)
        ...
        if recipe_list:
            ...
            if manage_action=="Edit":
                ...
                if recipe_edit:
                    ...
                    edit_difficulty=st.selectbox("Difficulty",['Easy','Medium','Hard'].index(selected_recipe["difficulty"]),key="edit_recipe_difficulty")
                    category_list=get_categories()
                    if category_list:
                        category_names=[cat["name"] for cat in category_list]
                        edit_category_name=st.selectbox("Category",category_names,index=category_names.index(next(cat["name"] for cat in category_list if cat["id"]==
                                                                                                                  selected_recipe["categroy id"])),key="edit_category_recipe")
    ...
    if st.button("Add a new category",key="add_category_button"):
    ...
    if category_list:
        ...
        if manage_action=="Edit":
            ...
            category_to_delete=st.selectbox("Select category to delete",category_name,key="delete_category_select")
