...
# Renamed function to avoid name conflict with route handler
def update_item_in_db(item_id: int, item: Item) -> Optional[Item]:
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    conn.close()
    if updated == 0:
        return None  # No rows were updated, meaning the item was not found
    item.id = item_id
    return item
